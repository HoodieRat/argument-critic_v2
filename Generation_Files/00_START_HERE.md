...
17. `16_MINIMAL_IMPLEMENTATION_PLAN.md`
18. `17_IMPLEMENTATION_RETROSPECTIVE.md`
19. `generation_manifest.json`

## Non-Negotiable Outcome

Build a stable, local, polished desktop-first argument companion that:
- uses an Electron desktop shell as the primary user surface
- keeps the browser extension path only as an optional legacy compatibility route
...
- supports clearing the active question queue and turning question generation on or off
- lets the user speak directly to the database
- uses procedural answers whenever exactness is better than AI
- supports screenshot/crop intake plus first-class file attachments in chat
- optionally ingests GPT-Researcher results if enabled
- persists sessions, questions, contradictions, objections, and reports
- ships with one easy installer path for normal Windows users and a separate source-checkout path for developers
