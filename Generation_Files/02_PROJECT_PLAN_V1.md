...
The project is a desktop-first local system.

1. Electron Desktop Shell
   - Primary user-facing shell
   - Desktop drawer window
   - Screenshot capture and crop workflow
   - Renderer UI hosting the shared sidepanel surface
   - Bundled local companion startup in packaged installs
...
3. Optional Legacy Extension Path
   - retained only for compatibility and debugging
   - managed Chrome launch path for unpacked extension use when explicitly needed

4. Runtime Launcher and Supervisor
   - packaged Windows installer for end users
   - source-checkout start path for developers
...
### One-command startup
The finished project must provide two clear entry paths:
- a packaged Windows installer for end users that launches the desktop app directly
- a simple root start command for source checkouts that installs or verifies prerequisites, starts the local server, launches the desktop shell, and reports readiness clearly
